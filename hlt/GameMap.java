...
public class GameMap {
    ...
    public Direction naiveNavigate(final Ship ship, final Position destination, ArrayList<Boolean> back) {
       ...
       else if(at(targetPos).isOccupied() && at(targetPos).ship.amoved==false && at(targetPos).ship.halite>=at(targetPos).halite/10 && (back.get(ship.id.id).equals(new Boolean(true)) || back.get(at(targetPos).ship.id.id).equals(new Boolean(true)))) {
           ship.amoved=true;
           at(targetPos).markUnsafe(ship);
           at(ship).markUnsafe(null);
           return direction;
           }
        }
        ship.amoved=true;
